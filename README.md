...
A small, offline-ready browser interface for the HOST device. The shelf presents each file as a horizontal book spine; selecting one opens its details. The message board uses a restrained typographic grid and a simple composer. Both views use the same dark ground and typography as the HOST teaser.
